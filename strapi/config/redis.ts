...
import RedisStoreFactory from 'koa-redis';
...
export const createRedisSessionStore = (env?: StrapiEnvAccessor): KoaSessionStore => {
  ...
  return {
    async get(key) {
      ...
      return (session ?? undefined) as KoaSessionState | undefined;
    },
    async set(key, session, maxAge, options) {
      await store.set(key, session, maxAge, options);
    },
    async destroy(key) {
