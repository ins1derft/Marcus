...
-   **Кеш и очереди (Redis):** Redis тоже поднимается в контейнере. Он
    ...
    основной поток запросов. Для dev/stage окружений в `.env` оставляем
    дефолты `REDIS_HOST=redis`, `REDIS_PORT=6379`, `REDIS_DB=0`,
    `REDIS_TLS=false` — compose поднимет контейнер в приватной сети (порт
    6379 маппим наружу только в dev для диагностики). В проде планируем
    управляемый Redis (Aiven, Upstash, Selectel и т.п.), поэтому заполняем
    хост/порт/учётные данные и включаем TLS (`REDIS_TLS=true`). Для
    дебага соединений пригодится `redis-cli --pass $REDIS_PASSWORD MONITOR`
    (запускается из контейнера или bastion-хоста).
